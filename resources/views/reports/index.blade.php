...
<div class="row g-4">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0"><i class="ri-test-tube-line me-2"></i>API Test Sonuçları</h5>
                <small class="text-muted">Trunçgil API versiyonları test sonuçları</small>
            </div>
            <div class="card-body">
                <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <div class="card border">
                                <div class="card-header d-flex justify-content-between align-items-center py-2">
                                    <h6 class="mb-0 fw-bold">{{ strtoupper($version) }}</h6>
                                ...
                                <div class="card-body py-2">
                                    <div class="small text-muted mb-2">
                                        ...
                                        <code class="small">{{ $result['url'] }}</code>
                                    </div>
                                    
                                    @if($result['status'] === 'success' && $result['rates'])
                                        <div class="row g-2">
                                            <div class="col-6">
                                                <div class="bg-success-subtle p-2 rounded text-center">
                                                    <div class="small text-muted">USD</div>
                                                    <div class="fw-bold text-success">{{ number_format($result['rates']['USD'], 4) }}</div>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="bg-warning-subtle p-2 rounded text-center">
                                                    <div class="small text-muted">EUR</div>
                                                    <div class="fw-bold text-warning">{{ number_format($result['rates']['EUR'], 4) }}</div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Düzenleme butonu -->
                                        <div class="mt-2">
                                            <button class="btn btn-outline-primary btn-sm w-100" onclick="editRates('{{ $version }}', {{ $result['rates']['USD'] }}, {{ $result['rates']['EUR'] }})">
                                                <i class="ri-edit-line me-1"></i>Düzenle
                                            </button>
                                        </div>
                                    @else
                                        <div class="alert alert-danger py-2 mb-2">
                                            <small><strong>Hata:</strong> {{ $result['error'] ?? 'Bilinmeyen hata' }}</small>
                                        </div>
                                        
                                        <!-- Manuel giriş butonu -->
                                        <button class="btn btn-outline-danger btn-sm w-100" onclick="editRates('{{ $version }}', 41.29, 48.55)">
                                            <i class="ri-add-line me-1"></i>Manuel Gir
                                        </button>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="mt-3 p-3 bg-info-subtle rounded">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <strong>Şu anda aktif:</strong> 
                            @if(isset($exchangeRates['api_version']) && $exchangeRates['api_version'] !== 'none')
                                <span class="badge bg-info">{{ strtoupper($exchangeRates['api_version']) }}</span>
                                <small class="text-muted ms-2">USD: {{ number_format($exchangeRates['USD'] ?? 0, 4) }} | EUR: {{ number_format($exchangeRates['EUR'] ?? 0, 4) }}</small>
                            @else
                                <span class="badge bg-warning">Manuel Giriş Gerekli</span>
                            @endif
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Edit rates function
    function editRates(version, currentUsd, currentEur) {
        const modalHtml = `
            <div class="modal fade" id="editRatesModal" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Döviz Kurları Düzenle - ${version.toUpperCase()}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="row g-3">
                                <div class="col-6">
                                    <label for="editUsdRate" class="form-label">USD Kuru</label>
                                    <input type="number" class="form-control" id="editUsdRate" 
                                           value="${currentUsd}" step="0.0001" min="0">
                                </div>
                                <div class="col-6">
                                    <label for="editEurRate" class="form-label">EUR Kuru</label>
                                    <input type="number" class="form-control" id="editEurRate" 
                                           value="${currentEur}" step="0.0001" min="0">
                                </div>
                            </div>
                            <div class="mt-3">
                                <small class="text-muted">
                                    <i class="ri-information-line me-1"></i>
                                    Bu kurlar manuel olarak ayarlanacak ve cache'e kaydedilecek.
                                </small>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">İptal</button>
                            <button type="button" class="btn btn-primary" onclick="saveManualRates('${version}')">Kaydet</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function saveManualRates(version) {
        const usdRate = parseFloat($('#editUsdRate').val());
        const eurRate = parseFloat($('#editEurRate').val());
        
        if (isNaN(usdRate) || isNaN(eurRate) || usdRate <= 0 || eurRate <= 0) {
            alert('Lütfen geçerli kur değerleri girin.');
            return;
        }
        
        // Show loading
        const saveBtn = $('#editRatesModal .btn-primary');
        const originalText = saveBtn.text();
        saveBtn.prop('disabled', true).text('Kaydediliyor...');
        
        // AJAX request to save rates
        $.ajax({
            url: '{{ route("reports.test-currency") }}',
            method: 'GET',
            data: {
                manual: true,
                usd: usdRate,
            ...
            success: function(response) {
                if (response.success) {
                    alert('Kurlar başarıyla kaydedildi! Sayfa yenileniyor...');
                    location.reload();
                } else {
            ...
            error: function() {
                alert('Kurlar kaydedilirken bir hata oluştu.');
            },
            complete: function() {
