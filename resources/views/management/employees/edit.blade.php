        ...
        <form action="{{ route('management.employees.update', $employee->id) }}" method="POST">
            ...
            <div class="row gy-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="date" class="form-control radius-8 @error('start_date') is-invalid @enderror" name="start_date" value="{{ old('start_date', $employee->start_date ? \Carbon\Carbon::parse($employee->start_date)->format('Y-m-d') : '') }}">
                    @error('start_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control radius-8 @error('address') is-invalid @enderror" name="address" rows="3" placeholder="Adres bilgilerini girin">{{ old('address', $employee->address) }}</textarea>
                    @error('address')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
